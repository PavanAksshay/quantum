"""
RoBERTa Dense Contextual Representation.
Uses frozen roberta-base (768D) with mean pooling followed by TruncatedSVD + StandardScaler.
"""
...
class RoBERTaRepresentation(BaseRepresentation):
    """
    Canonical RoBERTa-base dense contextual embedding model.
    Pipeline: roberta-base (768D, frozen) -> TruncatedSVD (d-dim) -> StandardScaler.
    """

    def __init__(self, target_dimension: int = 8):
        super().__init__(
            id="roberta",
            name="RoBERTa-base Contextual",
            representation_type="Deep Contextual Transformer (Dense)",
            is_sparse=False,
            original_dimension=768,
            target_dimension=target_dimension,
            description="Deep bidirectional transformer encoding semantic context via 768-dimensional frozen embeddings.",
            preprocessing_details="roberta-base (frozen weights, mean-pooling) -> TruncatedSVD -> StandardScaler",
            is_canonical=True
        )
        self.tokenizer = None
        self.model = None
    ...
    def _ensure_model_loaded(self):
        ...
        self.status_message = "Loading roberta-base weights into memory..."
        try:
            ...
            self.status_message = "roberta-base (768D) loaded and frozen"
        except Exception as e:
            self.status = ModelStatus.UNAVAILABLE
            self.status_message = f"roberta-base unavailable locally: {str(e)}"
            raise e
    ...
    def fit(self, texts: List[str], y: Optional[np.ndarray] = None) -> "RoBERTaRepresentation":
        """Fits TruncatedSVD and StandardScaler on dense RoBERTa embeddings."""
        try:
    ...
    def transform(self, texts: List[str], target_dim: Optional[int] = None) -> np.ndarray:
        """Transforms text to standardized projected RoBERTa vector."""
        if not self.is_fitted:
            raise RuntimeError("RoBERTaRepresentation is not fitted.")
