"""
MiniLM Sentence-Transformer Representation (Exploratory).
Uses frozen all-MiniLM-L6-v2 (384D) followed by TruncatedSVD + StandardScaler.
"""
...
class MiniLMRepresentation(BaseRepresentation):
    """
    Sentence-Transformer all-MiniLM-L6-v2 dense semantic embedding model.
    Pipeline: all-MiniLM-L6-v2 (384D, frozen) -> TruncatedSVD (d-dim) -> StandardScaler.
    """

    def __init__(self, target_dimension: int = 8):
        super().__init__(
            id="minilm",
            name="Sentence-MiniLM-L6",
            representation_type="Sentence-Transformer Semantic (Dense)",
            is_sparse=False,
            original_dimension=384,
            target_dimension=target_dimension,
            description="Compact 6-layer sentence transformer optimized for semantic similarity and semantic vector search.",
            preprocessing_details="sentence-transformers/all-MiniLM-L6-v2 (frozen, mean-pooling) -> TruncatedSVD -> StandardScaler",
            is_canonical=False
        )
        self.tokenizer = None
        self.model = None
    ...
    def _ensure_model_loaded(self):
        ...
        self.status_message = "Loading sentence-transformers/all-MiniLM-L6-v2 weights..."
        try:
            ...
            self.status_message = "all-MiniLM-L6-v2 (384D) loaded and frozen"
        except Exception as e:
            self.status = ModelStatus.UNAVAILABLE
            self.status_message = f"all-MiniLM-L6-v2 unavailable locally: {str(e)}"
            raise e

    def encode_original(self, texts: List[str]) -> np.ndarray:
        """Extracts dense 384D mean-pooled embeddings."""
    ...
    def fit(self, texts: List[str], y: Optional[np.ndarray] = None) -> "MiniLMRepresentation":
        """Fits TruncatedSVD and StandardScaler on dense MiniLM embeddings."""
        try:
            ...
            self.is_fitted = True
            self.status = ModelStatus.READY
            self.status_message = f"Fitted on {len(texts)} samples (384D -> TruncatedSVD)"
        except Exception as e:
    ...
    def transform(self, texts: List[str], target_dim: Optional[int] = None) -> np.ndarray:
        """Transforms text to standardized projected MiniLM vector."""
        if not self.is_fitted:
            raise RuntimeError("MiniLMRepresentation is not fitted.")
